 ...
 * @param data
 */
export async function getLatestVersion(data: ToolData): Promise<string> {
 ...
 * @param data
 */
export async function getUrl(data: ToolData): Promise<string> {
...
export async function getData(
  ...
  if (Object.hasOwn(json_objects, tool)) {
    ...
    if (key) {
      ...
      config = !tool.includes('/') ? {tool: tool} : config;
    }
  }
  ...
  const data: ToolData = {
    ...
    version_parameter: JSON.stringify(config.version_parameter) ?? '',
